...
def display_effect_all_comb(df, selg, conds, eff):
    effdic = {tag : valsincolumn(df, 'event_time_' + tag) for tag in ["death", "inc", "nic"]}
    tags = []
    for tag in effdic:
        if eff in effdic[tag]:
    ...
    #(dic, datadic) = get_cond_stats_comb(df, conds, selg, tags, eff)
    (dic, dummy1, dummy2) = get_cond_stats_daydiff(df, [], conds, selg, tags, eff)
    print(name)
    tscale = 1.0/36525
    sum0 = 0
    sum1 = 0
